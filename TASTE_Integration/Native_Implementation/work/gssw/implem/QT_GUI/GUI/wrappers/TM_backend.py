...
def setMsgQ():
    global tmId
    tmId = PythonController.i_TM


def setUDP():
    global tmId
    from _InterfaceEnum import i_gssw_PI_TM
    tmId = i_gssw_PI_TM
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.TM_Type_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.TM_Type_REQUIRED_BYTES_FOR_ENCODING)
    ...
    while True:
        ...
        if msgId == tmId:
            ...
            if asn1_python.compareVnValues(receivedValue, expectedValue):
                ...
                Q.task_done()
                raise ValueError('Received TM with wrong data: '+ vn.format_gser(receivedValue))
        elif not ignoreOther:
            Q.task_done()
            raise TypeError(
                "Expected TM (%s), but received other (%s)" % (str(tmId), str(msgId)))
        else:
            print('Received other (%s), but still waiting for TM') % str(msgId)
            Q.task_done()


def fromPysideToASN1(_):
    print('DEPRECATED call to "fromPysideToASN1"')
    # Create a native ASN.1 variable of type TM_Type
    # TM = ASN1.TM_Type()
    return None
