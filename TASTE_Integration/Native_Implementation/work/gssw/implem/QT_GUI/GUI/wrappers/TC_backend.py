...
def setUDP():
    global udp
    global tcId
    from _InterfaceEnum import i_gssw_RI_TC
    tcId = i_gssw_RI_TC
    udp = True
...
def setSharedLib(dll=None):
    ...
    global shared_lib
    global TC_via_shared_lib
    shared_lib = True
    TC_via_shared_lib = dll.gssw_PI_TC
...
def encode_ACN(asnVal):
    ...
    buffer = ASN1.DataStream(DV.TC_Type_REQUIRED_BYTES_FOR_ACN_ENCODING)
...
def encode_uPER(asnVal):
    ...
    buffer = ASN1.DataStream(DV.TC_Type_REQUIRED_BYTES_FOR_ENCODING)
...
def fromPysideToASN1(_):
    ...
    # Create a native ASN.1 variable of type TC_Type
    # TC = ASN1.TC_Type()
    return None
